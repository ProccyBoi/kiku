...
parts={p.stem:cq.Shape.importBrep(str(p)) for p in (ROOT/'exports/P1').glob('*.brep') if p.stem in inventory or p.stem.startswith('CHECK_')}
meshes={}
def mesh(n):
    if n not in meshes:
...
for p in (ROOT/'exports/P1').glob('*.stl'):
...
(ROOT/'review/clearance_review_P1.json').write_text(json.dumps(report,indent=2))
