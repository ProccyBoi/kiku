...
def render(files,path,view='iso',size=(1100,1250),explode=False,section=None):
    ...
    for name,fn in files:
        ...
        if name.startswith(('REF_FM_antenna','REF_SMA_')):col=(.64,.65,.64)
        if name=='REF_FM_100mm_RF113_coax':col=(.07,.085,.075)
        ...
        if name=='REF_PCB' and 'antenna_route' in path.name:actor.GetProperty().SetOpacity(.13)
        if explode:
            dz=0
            ...
            elif name.startswith(('REF_FM_antenna','REF_SMA_')):dz=-42
    ...
    if path.name.startswith('P1_assembled'):camera.SetParallelScale(80)
    if 'antenna_extended' in path.name:
        camera.SetFocalPoint(-132,53,0);camera.SetPosition(-132,53,250);camera.SetParallelScale(105)
...
def final_views():
    inventory=json.loads((ROOT/'exports/P1/part_inventory.json').read_text())
    files=[(p.stem,p) for p in (ROOT/'exports/P1').glob('*.brep') if p.stem in inventory]
    outer=[p for p in files if p[0] in COL or p[0].startswith(('REF_FM_antenna','REF_SMA_'))]
    if '--sections' not in sys.argv:
        for view in ('front','rear','side','iso'):render(outer,ROOT/f'review/P1_assembled_{view}.png',view)
        extended=[p for p in outer if p[0]!='REF_FM_antenna_collapsed_BOUND']+[('REF_FM_antenna_extended_BOUND',ROOT/'exports/P1/CHECK_FM_extended_outward.brep')]
        render(extended,ROOT/'review/P1_antenna_extended.png','front',size=(2000,900))
        if '--outer-only' in sys.argv:return
        render(files,ROOT/'review/P1_exploded.png','iso',size=(1600,1600),explode=True)
    antenna_parts=[p for p in files if p[0] in ('rear_shell','REF_PCB','REF_FM_100mm_RF113_coax','REF_J3','REF_J3_mated_UFL_ENVELOPE') or p[0].startswith(('REF_FM_antenna','REF_SMA_'))]
    render(antenna_parts,ROOT/'review/P1_antenna_route.png','inside_rear',size=(1500,1500))
    render(files,ROOT/'review/P1_section_longitudinal.png','side',size=(1300,1500),section=((30,0,0),(-1,0,0)))
    render(files,ROOT/'review/P1_section_controls.png','iso',size=(1500,1500),section=((0,55,0),(0,1,0)))
    render(files,ROOT/'review/P1_section_speaker.png','side',size=(1300,1500),section=((18,0,0),(-1,0,0)))
    render([p for p in files if p[0]=='front_shell'],ROOT/'review/P1_front_inside.png','inside_front')
    render([p for p in files if p[0]=='rear_shell'],ROOT/'review/P1_rear_inside.png','inside_rear')
